...
切换：`start-fastllm`。日常模型名：`qwen38-fastllm-nvfp4-mtp3-cn-210k`（**NVFP4 + FP16 KV + 240K + 关思考，带图像**）。端口 `:8000`。不要写成「纯文本档」——那是 v0.2 的 `text-only` profile。
...
这套没有 v0.2 那种 cubin 双缓存。启动快，热重启大约几十秒。
...
| 和 v0.2 同卡双开 | 不能 | 两张卡只能跑一套 |

长中文实请求（精确 64K / 128K / 200K 输入，出 128 token，关思考）：NVFP4 + MTP3 三档预填充和总耗时都优于当时的 v0.2。200K decode 会掉到约 27.6 tok/s。
...
没有做的事也写清楚：没有给 FastLLM 打 SM75 兼容补丁，没有把 DFlash 设成默认生产（本机 MTP3 更快），没有和 v0.2 做同尺质量赛。

## 启动注意

- 单请求服务，并发先留 1。`max_batch=1` 下多人是排队，不是批处理。
- `docker restart` 大约 46 秒能回来，适合热重启。
- 用 `start-fastllm`，不要和 v0.2 手搓双开。
...
- **长输入端到端它更快。** 相对 v0.2，64K / 128K / 200K 总耗时大约少 47% / 5% / 6%。64K 等第一口，差距最明显。
...
大段中文贴进去、等第一口出来，优先这套。后面还要持续吐得快，切到 v0.2 对照。质量结论目前只对 FastLLM 自己的 FP8 / NVFP4 负责。
